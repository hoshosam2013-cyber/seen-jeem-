...
import { fetchRemainingRounds, handleLogin, signOut, supabase } from '../services/supabase';
...
const Setup: React.FC<SetupProps> = ({ onStart }) => {
  ...
  // حالة المستخدم (من Supabase Auth)
  const [user, setUser] = useState<any>(null);

  const [selectedIds, setSelectedIds] = useState<string[]>([]);

  useEffect(() => {
    // 1. التحقق من الجلسة الحالية عند فتح الموقع
    supabase.auth.getSession().then(({ data: { session } }) => {
      setUser(session?.user || null);
      if (session?.user?.email) {
        loadRounds(session.user.email);
      }
    });

    // 2. الاستماع لتغييرات حالة تسجيل الدخول
  ...
  const loadRounds = async (email: string) => {
    setIsLoadingRounds(true);
    const data = await fetchRemainingRounds(email);
    setRoundsData(data);
  ...
  const toggleCategory = (id: string, isAvailable: boolean) => {
    if (!user) {
        audio.playError();
        alert("يرجى تسجيل الدخول لرؤية الفئات المتاحة.");
        return;
    }
    if (!isAvailable) {
      audio.playError();
    ...
    <div className="w-full max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 p-6 min-h-screen items-stretch animate-in fade-in duration-500">
      <div className="lg:col-span-4 xl:col-span-3 space-y-6">
        <div className="glass rounded-[2rem] p-8 shadow-2xl space-y-8 sticky top-6">
          <div className="text-center space-y-2 relative">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                {isLoadingRounds ? 'جاري المزامنة...' : user ? 'متصل بالسحابة' : 'غير متصل'}
              </p>
            </div>
          </div>

          {/* قسم المستخدم - يعتمد على Google Auth */}
          <div className="bg-slate-950/60 rounded-3xl p-6 border border-white/5 space-y-4 shadow-inner relative overflow-hidden group">
            {!user ? (
              <div className="space-y-4">
                 <p className="text-[10px] text-slate-500 text-center font-black uppercase tracking-widest">سجل دخولك لحفظ التقدم</p>
                 <button 
                  onClick={handleLogin}
                  className="w-full py-4 bg-white text-slate-900 rounded-xl font-bold hover:bg-slate-200 transition-all flex items-center justify-center gap-3 shadow-lg"
                 >
                   <img src="https://www.google.com/favicon.ico" alt="Google" className="w-5 h-5" />
                   <span>دخول عبر Google</span>
                 </button>
                 <p className="text-[9px] text-slate-600 text-center">يتم استخدام حسابك لضمان عدم تكرار الأسئلة.</p>
              </div>
            ) : (
              <div className="flex items-center gap-4">
                <div className="relative">
                    {user.user_metadata?.avatar_url ? (
                      <img src={user.user_metadata.avatar_url} alt="Avatar" className="w-14 h-14 rounded-full border-2 border-yellow-500 shadow-lg" />
                    ) : (
                      <div className="w-14 h-14 rounded-full border-2 border-yellow-500 shadow-lg bg-slate-800 flex items-center justify-center text-2xl">👤</div>
                    )}
                    <div className="absolute -bottom-1 -right-1 bg-green-500 w-4 h-4 rounded-full border-2 border-slate-900"></div>
                </div>
                <div className="flex-1 overflow-hidden">
                   <p className="text-sm font-black text-white truncate">{user.user_metadata?.full_name || user.email}</p>
                   <p className="text-[9px] text-green-500 truncate font-bold">الحساب نشط ✓</p>
                   <button onClick={signOut} className="text-[10px] text-red-500 hover:text-red-400 font-bold mt-1 underline">تسجيل الخروج</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4 pt-4">
            ...
            >
              {user ? 'ابدأ التحدي 🚀' : 'سجل الدخول أولاً'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="lg:col-span-8 xl:col-span-9 glass rounded-[2.5rem] p-6 flex flex-col shadow-2xl border-white/5">
        <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          <div>
            <h2 className="text-2xl font-black text-white italic tracking-tight">اختر فئات اللعب</h2>
                ...
                {CATEGORIES.filter(c => c.group === group).map(cat => {
                  ...
                  return (
                    <div key={cat.id} className="relative group flex flex-col gap-2">
                      ...
                      >
                        <img 
                          src={cat.imageUrl || 'https://via.placeholder.com/300x400?text=No+Image'} 
